...
select * from driver_id;
select * from pessenger_id;
select * from ride_id;
...
SELECT driver_id, SUM(total_rides) as earnings 
FROM driver_id 
GROUP BY driver_id 
...
select fare_amount from ride_id  where fare_amount >20;


#Q7: Identify the most common pickup location.
select pickup_location  , count(*)  as most_comman_pickup_locations
from ride_id 
group by pickup_location 
order by most_comman_pickup_locations desc  limit 1;
...
select driver_id , sum(earnings) as total_earning
from driver_id
group by driver_id 
order by total_earning desc;
select sum(earnings ) from driver_id; 

select * from driver_id ;

#Q11: How many rides were paid using the "Cash" payment method?
select ride_id from ride_id  where payment_method ="cash"
...
select pickup_location  , count(*), avg(ride_distance) from ride_id where pickup_location ="Dhanbad" ;
...
select  ride_duration,ride_id  from ride_id where ride_duration =10;

#Q14: List the passengers who have taken the most number of rides.
SELECT passenger_id, COUNT(*) AS ride_count 
FROM ride_id rc GROUP BY passenger_id 
ORDER BY ride_count DESC LIMIT 1;
select passenger_id , count(*) ride_count
from ride_id 
...
select driver_id , count(*) driver_ride_count
from ride_id
group by driver_id 
order by  driver_ride_count desc;
...
select distinct payment_method , count(*)  gandhinagar
from ride_id
group by payment_method 
order by gandhinagar desc;
...
SELECT pickup_location, COUNT(*) AS ride_id, ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM ride_id , 2) AS percentage 
from ride_id ri  
GROUP BY pickup_location
...
select ride_timestamp, (STR_TO_DATE(ride_timestamp)) from ride_id;
...
FROM driver_id 
WHERE driver_id IN (SELECT driver_id FROM ride_id GROUP BY driver_id HAVING COUNT(*) > 100)
GROUP BY driver_id order by total_earnings  desc;
...
SELECT * 
FROM ride_id 
WHERE fare_amount < (SELECT AVG(fare_amount) FROM ride_id );


SELECT AVG(fare_amount) FROM ride_id ;
...
(SELECT driver_id FROM ride_id GROUP BY driver_id  HAVING COUNT(*) >= 100);

select dc.driver_id, avg(rating) as Avg_Rating
from driver_id di  dc inner join rides_date_csv rdc 
